...
/*	{"APP-L2E", 60, app_l2E_process, -1},
	...
	{"APP-L3B2", 60, app_l3B2_process, -1},
...
static int app_l2A_process()
{
	...
	while(1){
		...
		strncpy(buffer.ifr_name, "tap-G0.0", IFNAMSIZ);
...
static int app_l2E_process()
{
	...
	while(1){
		memset(&buffer, 0x00, sizeof(buffer));
		strncpy(buffer.ifr_name, "tap-intfV1", IFNAMSIZ);
		if (ioctl(s, SIOCGIFINDEX, &buffer) < 0) {
			printf("Error: could not get interface index\n");
			close(s);
			return -1;
...
static int app_l3B2_process()
{
	...
	while(1){
		...
		offset = 20;
		memcpy(&addr, &from.sin_addr.s_addr, 4);
		sprintf(desc, "APP-L3B2 RX from %s", (char *)inet_ntoa(addr));
		dump_frame(desc, buffer + offset, n_read - offset);
	}
