...
int send_packet(int sock, struct tftpx_packet *packet, int size){
	...
	for(rxmt = 0; rxmt < PKT_MAX_RXMT; rxmt ++){//最大包重传次数
		printf("Send block=%d\n", ntohs(packet->block));
		if(send(sock, packet, size, 0) != size){
			return -1;
		}
...
void *work_thread(void *arg){
	int sock;
	struct tftpx_request *request = (struct tftpx_request *)arg;//强转记录请求。
	...
	if((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0){//建立sock
	...
	server.sin_port = 0;//端口不对吧

	if (bind(sock, (struct sockaddr *)&server, sizeof(server)) < 0){//绑定sock与服务器地址server
		printf("work_thread bind failed.\n");
		return NULL;
	}
	
	if(connect(sock, (struct sockaddr*)&(request->client), addr_len) < 0){//判断server是否与client保持得有连接
		printf("Can't connect to client.\n");
		return NULL;
	}
	
	// Choose handler
	switch(request->packet.cmd){//判断请求命令
		case CMD_RRQ:
			printf("handle_rrq called.\n");
			handle_rrq(sock, request);//处理请求，向下传sock，request
			break;
		case CMD_WRQ:
...
void handle_rrq(int sock, struct tftpx_request *request){//读请求，准备发送错误或文件
	struct tftpx_packet snd_packet;
	char fullpath[256];
	char *r_path = request->packet.filename;	// request file
	char *mode = r_path + strlen(r_path) + 1;//找到mode位置
	char *blocksize_str = mode + strlen(mode) + 1;//暂时不用
	int blocksize = atoi(blocksize_str);

	if(blocksize <= 0 || blocksize > DATA_SIZE){//确定数据块的大小
		blocksize = DATA_SIZE;
	}

	if(strlen(r_path) + strlen(conf_document_root) > sizeof(fullpath) - 1){	//文件名过长判断	
		printf("Request path too long. %d\n", strlen(r_path) + strlen(conf_document_root));
		return;
	}
	...
	// build fullpath
	memset(fullpath, 0, sizeof(fullpath));
	strcpy(fullpath, conf_document_root);//追加.
	if(r_path[0] != '/'){
		strcat(fullpath, "/");
	}
	strcat(fullpath, r_path);
	...
	FILE *fp = fopen(fullpath, "r");//打开文件路径
	if(fp == NULL){
	...
	do{
		memset(snd_packet.data, 0, sizeof(snd_packet.data));//初始化将发包的内容
		snd_packet.block = htons(block);//设置包号
		s_size = fread(snd_packet.data, 1, blocksize, fp);//用二进制方式重文件中些包大小的内容到将发送的包的数据中
